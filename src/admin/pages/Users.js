...
import { FaSearch, FaUserPlus, FaEnvelope, FaPhone, FaUserEdit, FaUserTimes, FaFilter, FaDownload, FaEye, FaEyeSlash, FaSort, FaSortUp, FaSortDown, FaInfoCircle } from 'react-icons/fa';
...
export default function Users() {
  ...
  const { user: currentUser, refreshUserSession, isAdmin, getToken } = useAuth();
  const attemptedSessionRefreshRef = useRef(false);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        // Get the appropriate token for admin requests
        const token = getToken();
        if (!token) {
          throw new Error('No authentication token found. Please log in again.');
        }
        
        // Refresh user session to ensure we have the latest data
        const sessionRefreshed = refreshUserSession();
        if (!sessionRefreshed) {
        ...
        if (err.message.includes('Authentication failed') || err.message.includes('No authentication token') || err.message.includes('User session not found')) {
          setTimeout(() => {
    ...
    if (currentUser) {
      // Block unverified or non-admin access early to avoid forbidden API calls
      if (!isAdmin()) {
      ...
      if (!currentUser.is_email_verified) {
        setError('Admin email not verified. Please verify your email.');
        setLoading(false);
        return;
      }
      fetchUsers();
    ...
    // No current user: only attempt one refresh to avoid infinite loop
    if (!attemptedSessionRefreshRef.current) {
      attemptedSessionRefreshRef.current = true;
      try {
        const refreshed = refreshUserSession();
        if (refreshed) return; // effect will re-run when currentUser updates
        setError('User session not found. Please log in again.');
      } finally {
        setLoading(false);
      }
    } else {
      // Already attempted refresh; stop loading and show error if not set
      if (!error) setError('User session not found. Please log in again.');
      setLoading(false);
    }
